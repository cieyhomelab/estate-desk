...
export const POST: APIRoute = async (context) => {
  ...
  if (!(file instanceof File) || file.size === 0) {
    return context.redirect(`/dashboard/listings/${id}/documents?error=brak-pliku`);
  }
  ...
  const { error: dbError } = await supabase.from("listing_files").insert({
    listing_id: id,
    user_id: user.id,
    file_name: file.name,
  ...
  return context.redirect(`/dashboard/listings/${id}/documents?success=dodano-plik`);
};
