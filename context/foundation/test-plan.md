...
|---|---|---|---|---|---|---|
| 1 | Bootstrap + commission arithmetic | Install Vitest; unit-test commission formula — first signal at the cheapest layer | #2 | unit | change opened | context/changes/testing-bootstrap-commission/ |
| 2 | Persistence + data lifecycle | Integration tests: listing save/reload (R1), price history ordering (R5), completed listing close/reopen cycle (R4) | #1, #4, #5 | integration (real DB) | not started | — |
...
TBD — see §3 Phase 2. Target pattern: API write → real Supabase → DB read-back assertion; mock network edge only.
